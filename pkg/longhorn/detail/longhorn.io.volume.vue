...
export default {
  ...
  async fetch() {
    ...
    if (inStore) {
      const [backingImages] = await Promise.all([
        this.$store.dispatch(`${inStore}/findAll`, { type: LONGHORN_RESOURCES.BACKING_IMAGES }),
        this.$store.dispatch(`${inStore}/findAll`, { type: LONGHORN_RESOURCES.ENGINES }),
      ]);
  ...
  computed: {
    ...
    backingImageInfo() {
      ...
      return { name: name || 'None', size: bi?.spec?.size ? this.formatBinary(bi.spec.size) : '—' };
    },
...
<template>
  ...
  <Tabbed v-else :side-tabs="true" :resource="value">
    <Tab name="details" label="Details" :weight="100">
          ...
          <LabelValue name="Size">
            <template #name>
              <span
                >Size
                <i
                  v-clean-tooltip="'Amount of space specified during creation.'"
                  class="icon icon-info ml-5 text-muted"
              /></span>
            </template>
            <template #value>
              <span v-clean-tooltip="value.spec.size">{{ formattedSize }}</span>
            </template>
          </LabelValue>
        </div>
          ...
          <LabelValue name="Frontend">
            <template #value>
              <span v-if="value.spec.disableFrontend"
                ><i v-clean-tooltip="'Frontend disabled'" class="icon icon-disconnect text-success mr-5" />
                Disabled</span
              >
      ...
      <div class="row mb-20">
          ...
          <LabelValue name="Ready for Workload">
            <template #value>
              ...
              <span
                ...
                "
                class="text-error"
                >Not Ready <i class="icon icon-info ml-5"
              /></span>
            </template>
          </LabelValue>
        </div>
        <div class="col span-4">
          <LabelValue name="Actual Size">
            ...
            <template #value>
              <span v-clean-tooltip="value?.status?.actualSize || 0">{{ formattedActualSize }}</span>
            </template>
          </LabelValue>
        </div>
      </div>
      ...
      <div v-if="value.status.kubernetesStatus">
        <div class="row mb-20">
          <div v-if="k8sStatus.lastPVCRefAt" class="col span-4">
            <LabelValue name="Last time bound with PVC">
              <template #value>{{ getLiveDateLabel(k8sStatus.lastPVCRefAt) }}</template>
            </LabelValue>
          </div>
          <div v-if="k8sStatus.lastPodRefAt" class="col span-4">
            <LabelValue name="Last time used by Pod">
              <template #value>{{ getLiveDateLabel(k8sStatus.lastPodRefAt) }}</template>
            </LabelValue>
          </div>
          <div class="col span-4">
        ...
        <div class="row mb-20">
          ...
          <div class="col span-4">
            <LabelValue name="PV Name" :value="k8sStatus.pvName || '—'" />
          </div>
        </div>

        <div class="row mb-20">
          <div class="col span-4">
            <LabelValue name="PV Status" :value="k8sStatus.pvStatus || '—'" />
          </div>
          <div class="col span-4">
            <LabelValue name="Last Backup" :value="value.status.lastBackup || '—'" />
          </div>
        </div>
        ...
        <div v-for="(workload, index) in k8sStatus.workloadsStatus" :key="index" class="mb-20">
          <div class="row">
            <div class="col span-4">
              <LabelValue
                :name="(k8sStatus.lastPodRefAt ? 'Last ' : '') + 'Pod Name'"
                :value="workload.podName || '—'"
              />
            </div>
            <div class="col span-4">
              <LabelValue
      ...
      <div class="row mb-20">
        ...
        <div class="col span-6">
          <LabelValue name="Endpoint">
            <template #value>
              <span v-if="engine?.status?.endpoint" class="code-font">{{ engine.status.endpoint }}</span>
              <span v-else>—</span>
            </template>
          </LabelValue>
        </div>
        ...
        <div class="col span-6">
          <LabelValue name="Instance Manager">
            <template #name>
              <span
                >Instance Manager
                <i v-clean-tooltip="'Manages engine/replica life cycle.'" class="icon icon-info text-muted ml-5"
              /></span>
            </template>
    ...
    <Tab name="conditions" label="Conditions" :weight="90"><Conditions :value="value" /></Tab>
  </Tabbed>
</template>
...
.code-font {
  ...
  background: var(--nav-block-active);
  padding: 2px 4px;
  border-radius: 4px;
}
.text-success {
